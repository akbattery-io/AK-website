...
function ProductCard({ project, idx, onSelectProduct }: { project: any; idx: number; onSelectProduct: (p: any) => void }) {
  ...
  return (
    <motion.div
      ...
      className="group bg-white rounded-xl p-3.5 sm:p-4 shadow-[0_4px_20px_rgba(15,23,42,0.03)] border border-slate-200/80 hover:shadow-[0_14px_35px_rgba(15,23,42,0.08)] hover:border-rose-200 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between relative cursor-pointer overflow-hidden"
    >
      ...
      <div className="bg-slate-50/80 rounded-xl h-44 sm:h-48 w-full overflow-hidden relative flex items-center justify-center p-3 border border-slate-100 mb-3 group-hover:bg-slate-100/60 transition-colors duration-300">
        ...
        {discountBadge && (
          <span className="absolute top-2 left-2 bg-emerald-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider shadow-sm select-none">
            {discountBadge}
          </span>
        )}

        <span className="absolute bottom-2 right-2 bg-white/90 backdrop-blur-sm text-slate-700 text-[9px] font-bold px-2 py-0.5 rounded-full shadow-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
      ...
      <div className="flex flex-col flex-1 justify-between gap-2">
        <div className="space-y-1.5">
          ...
          <div className="flex items-center justify-between">
            <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider border select-none ${
            ...
            <span className="w-5 h-5 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-rose-50 group-hover:text-rose-600 transition-all duration-300 select-none">
        ...
        <div className="mt-2.5 pt-2 border-t border-slate-100 space-y-2">
          ...
          <button
            ...
            className="w-full bg-slate-900 hover:bg-slate-800 active:scale-[0.98] text-white rounded-lg py-2 px-3 text-[11px] font-extrabold uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all duration-300 shadow-sm cursor-pointer"
          >
        ...
        {initialProducts.length > 0 && (
          <div className="max-w-6xl mx-auto mb-12 flex flex-col md:flex-row md:items-center justify-between gap-4 px-4 sm:px-0">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
              ...
              {/* Price Sort Dropdown */}
              <div className="flex items-center gap-2 bg-white px-3.5 py-1.5 rounded-full border border-slate-200 shadow-[0_2px_10px_rgba(15,23,42,0.01)]">
              ...
              <div className="flex flex-wrap gap-2">
                {["All", "ups inventer & batteries", "water purifier"].map((cat) => (
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 border ${selectedCategory === cat
